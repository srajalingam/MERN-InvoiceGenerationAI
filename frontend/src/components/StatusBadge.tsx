...
const StatusBadge = ({
  ...
  return (
    <div className={`inline-flex items-center ${sizeClasses[size]} rounded-full font-medium ${config.bg} ${config.text} border ${config.border} relative overflow-hidden`}>
      <div className={`absolute inset-0 bg-linear-to-r ${config.gradient}`}></div>
      {showIcon && (
        <div className='relative z-10 flex items-center'>
          <IconComponent className="w-3 h-3"/>
        </div>
      )}
      <span className='relative z-10 first-letter:uppercase font-semifold'>
        {s=="default"?status:s}
      </span>
      {
        (s=='unpaid' || s=='overdue') && (
